...
CREATE TABLE IF NOT EXISTS `transport`.`ticket` (
  ...
  `returnTicket` TINYINT(4) NOT NULL DEFAULT '0',
  `date_of_journey` DATETIME NOT NULL,
  `time_of_journey` DATETIME NOT NULL,
  `fk_coordinate_id_to` INT(11) NOT NULL,
  `fk_coordinate_id_from` INT(11) NOT NULL,
  PRIMARY KEY (`ticket_id`),
  INDEX `fk_ticket_coordinate1_idx` (`fk_coordinate_id_to` ASC) ,
  INDEX `fk_ticket_coordinate2_idx` (`fk_coordinate_id_from` ASC) ,
  CONSTRAINT `fk_ticket_coordinate1`
    FOREIGN KEY (`fk_coordinate_id_to`)
    REFERENCES `transport`.`coordinate` (`coordinate_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_ticket_coordinate2`
    FOREIGN KEY (`fk_coordinate_id_from`)
    REFERENCES `transport`.`coordinate` (`coordinate_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `transport`.`user_journey` (
  ...
  `favourited` TINYINT(4) NOT NULL DEFAULT '0',
